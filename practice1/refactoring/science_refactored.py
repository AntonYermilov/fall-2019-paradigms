...
def format_person(p: Person):
    return '{}. {}, born on {}'.format(p.first_name[:1], p.last_name, p.birth_date)


def print_table(scientists: List[Person], title: str):
    print('--- {} ---'.format(title))
    for p in scientists:
        print('* {}'.format(format_person(p)))
    print('Total: {}\n'.format(len(scientists)))


print_table([p for p in people if p.area == 'physics'], title='Physics')

print()

print_table([p for p in people if p.area == 'cs'], title='Computer Science')

oldest_man = min(people, key=lambda p: p.birth_date)
print('Oldest person: {}'.format(format_person(oldest_man)))
